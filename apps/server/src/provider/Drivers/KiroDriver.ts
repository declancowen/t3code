/**
 * KiroDriver — `ProviderDriver` for Kiro CLI's ACP runtime.
 *
 * Kiro exposes Agent Client Protocol over `kiro-cli acp`, so the driver
 * composes the existing provider instance model with the shared ACP adapter
 * instead of introducing a provider-specific transport path.
 *
...
import { withLocalUserHome } from "../../localUserEnvironment.ts";
...
export const KiroDriver: ProviderDriver<KiroSettings, KiroDriverEnv> = {
  ...
  create: ({ instanceId, displayName, accentColor, environment, enabled, config }) =>
    Effect.gen(function* () {
      ...
      const baseEnv = withLocalUserHome(mergeProviderInstanceEnvironment(environment));
      const effectiveConfig = { ...config, enabled } satisfies KiroSettings;
      const processEnv = yield* makeKiroEnvironment(effectiveConfig, baseEnv);
