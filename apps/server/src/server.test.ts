...
const assertBrowserApiCorsHeaders = (
  ...
  if (options?.origin !== undefined) {
    ...
    assert.include(splitHeaderTokens(headers.vary ?? null), "Origin");
  }
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("does not reflect untrusted origins on remote websocket-token auth failures", () =>
    Effect.gen(function* () {
      ...
      assert.notInclude(splitHeaderTokens(response.headers.vary ?? null), "Origin");
      assert.equal(body.error, "Authentication required.");
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );
  ...
  it.effect("responds to browser OTLP trace preflight requests with CORS headers", () =>
    Effect.gen(function* () {
      ...
      assert.equal(response.status, 204);
      assertBrowserApiCorsHeaders(response.headers, { origin: "http://localhost:5733" });
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );
