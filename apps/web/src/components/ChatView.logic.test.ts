...
describe("hasServerAcknowledgedLocalDispatch", () => {
  ...
  it("does not acknowledge when a running session references a newer turn than latestTurn", () => {
    const localDispatch = createLocalDispatchSnapshot(
      makeThread({ latestTurn: completedTurn, session: readySession }),
    );
  ...
  it("does not acknowledge while running before latestTurn has advanced", () => {
    const localDispatch = createLocalDispatchSnapshot(
      makeThread({ latestTurn: completedTurn, session: readySession }),
    );
  ...
  it("acknowledges an additional prompt accepted by the active running turn", () => {
    ...
    const localDispatch = createLocalDispatchSnapshot(
      makeThread({ latestTurn: activeLatestTurn, session: activeSession }),
    );
  ...
  it("acknowledges a session change without an observed running phase", () => {
    const localDispatch = createLocalDispatchSnapshot(
      makeThread({ latestTurn: completedTurn, session: readySession }),
    );
