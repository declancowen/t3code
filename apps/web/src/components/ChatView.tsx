...
function ChatViewContent(props: ChatViewProps) {
  ...
  const removeThreadQueuedMessage = useAtomCommand(threadEnvironment.removeQueuedMessage, {
    reportFailure: false,
  });
  ...
  const onRemoveQueuedMessage = useCallback(
    async (messageId: MessageId) => {
      if (!activeThread) return;
      const result = await removeThreadQueuedMessage({
        environmentId,
        input: {
          threadId: activeThread.id,
          messageId,
    ...
    [activeThread, environmentId, removeThreadQueuedMessage],
  );

  const onEditQueuedMessage = useCallback(
    async (messageId: MessageId, text: string) => {
      if (!activeThread) return;
      const result = await editThreadQueuedMessage({
        environmentId,
        input: {
          threadId: activeThread.id,
          messageId,
  ...
  return (
    <div className="relative flex min-h-0 min-w-0 flex-1 overflow-hidden rounded-l-2xl bg-background">
