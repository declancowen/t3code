    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        assert.equal(
          ...
          "Electron updater failed to quit and install the update on channel default (silent: false, force run after: false).",
        );
        assert.equal(failingHarness.stopCount(), 1);
        assert.equal(failingHarness.quitAndInstallCount(), 1);
        assert.equal(failingHarness.destroyAllCount(), 1);
