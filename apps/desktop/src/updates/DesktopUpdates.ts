...
export const make = Effect.gen(function* () {
  ...
  const restartBackendsAfterInstallFailure = Effect.fn(
    "desktop.updates.restartBackendsAfterInstallFailure",
  )(function* () {
  ...
  const installDownloadedUpdate = Effect.gen(function* () {
    ...
    return yield* Effect.gen(function* () {
      ...
      const instances = yield* pool.list;
      yield* logUpdaterInfo("stopping desktop backends before update install", {
      ...
      // scope cascade has a chance to run its stop finalizer.
      yield* Effect.forEach(
        instances,
        (instance) => instance.stop({ timeout: INSTALL_BACKEND_STOP_TIMEOUT }),
        { concurrency: "unbounded" },
      );
